...
Fixed_Acidity = st.slider("Fixed Acidity", min_value=1992, max_value=2020, step=1)
st.text(f"Fixed acidity value: {Fixed_Acidity}")
Volatile_acidity = st.slider("Volatile_acidity", min_value=1, max_value=806599, step=1)
st.text(f"Volatile acidity value: {Volatile_acidity}")
citric_acid = st.slider("Citric Acid", min_value=0, max_value=5,step=1)
st.text(f"citric acid value: {citric_acid}")
Residual_sugar = st.slider("Residual Sugar", min_value=0, max_value=2,step=1)
st.text(f"Residual sugar value: {Residual_sugar}")
Chlorides = st.slider("Chlorides", min_value=0, max_value=1,step=1)
st.text(f"Chlorides value: {Chlorides}")
Free_sulfur_dioxide = st.slider("Free sulfur dioxide", min_value=0, max_value=4,step=1)
st.text(f"Free sulfur dioxide value: {Free_sulfur_dioxide}")
Total_sulfur_dioxide = st.slider("Total sulfur dioxide", min_value=0, max_value=28,step=1)
st.text(f"Total sulfur dioxide value: {Total_sulfur_dioxide}")
density = st.slider("density", min_value=0, max_value=1,step=1)
st.text(f"density value: {density}")
pH = st.slider("pH", min_value=0, max_value=1,step=1)
st.text(f"pH value: {pH}")
Sulphates = st.slider("Sulphates", min_value=0, max_value=1,step=1)
st.text(f"Sulphates value: {Sulphates}")
alcohol = st.slider("alcohol", min_value=0, max_value=1,step=1)
